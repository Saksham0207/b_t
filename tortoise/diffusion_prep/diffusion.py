...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output, model_output_no_conditioning = model(x, self._scale_timesteps(t),
                                                           model_kwargs["precomputed_aligned_embeddings"])
...
class _WrappedModel:
    ...
    def __call__(self, x, ts, precomputed_aligned_embeddings):
        map_tensor = th.tensor(self.timestep_map, device=ts.device, dtype=ts.dtype)
        new_ts = map_tensor[ts]
        if self.rescale_timesteps:
            new_ts = new_ts.float() * (1000.0 / self.original_num_steps)
        
        return self.model(x, new_ts, precomputed_aligned_embeddings)
